...
int main (int argc, char *argv[])
{
  ...
  std::string transport_prot2 = "TcpHighSpeed";
  double error_p = 0.0;
  ...
  cmd.AddValue ("transport_prot2", "Transport protocol2 to use: TcpNewReno, "
                "TcpHybla, TcpHighSpeed, TcpHtcp, TcpVegas, TcpScalable, TcpVeno, "
                "TcpBic, TcpYeah, TcpIllinois, TcpWestwood, TcpWestwoodPlus ", transport_prot2);
  ...
  //uint32_t size = (std::min (access_b, bottle_b).GetBitRate () / 8) * ((access_d + bottle_d) * 2).GetSeconds ();
  uint32_t size = mtu_bytes * 10;
  ...
  for (int i = 0; i < num_flows; i++)
    {
      ...
      //StartQueueTrace(devices.Get(0), "packets", prefix_file_name + "-queue-" + std::to_string(0) + ".data");
      //StartQueueTrace(devices.Get(1), "packets", prefix_file_name + "-queue-" + std::to_string(1) + ".data");
