...
CwndChange (uint32_t oldCwnd, uint32_t newCwnd)
{
  ...
  if (newCwndFile)
    {
      myfile.open ("new-reno-cwnd.log");
      newCwndFile = false;
    }
  else
    {
       myfile.open ("new-reno-cwnd.log", ios::out | ios::app);
    }
...
SsThreshChange (uint32_t oldSsThresh, uint32_t newSsThresh)
{
  ...
  if (newSsThresFile)
    {
      myfile.open ("new-reno-ssthresh.log");
      newSsThresFile = false;
    }
  else
    {
       myfile.open ("new-reno-ssthresh.log", ios::out | ios::app);
    }
