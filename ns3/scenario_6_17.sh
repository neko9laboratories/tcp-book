...
for dl in 10ms 100ms; do
  ...
  for flw in 0; do
	...
	set output 'data/06_xx-sc${sc}-$item-${bw}-${dl}-flw${flw}-cwnd.png'
	set xlabel "Time [s]"
  ...
  # RTT
	gnuplot <<- EOS
	set terminal pngcairo enhanced font "TimesNewRoman" fontscale 2.5 size 1280,960
	set output 'data/06_xx-sc${sc}-$item-${bw}-${dl}-flw${flw}-rtt.png'
	set xlabel "Time [s]"
	set ylabel "RTT [s]"
	set xrange [0:$time]
  ...
  # cong-state
	gnuplot <<- EOS
	set terminal pngcairo enhanced font "TimesNewRoman" fontscale 2.5 size 1280,960
	set output 'data/06_xx-sc${sc}-$item-${bw}-${dl}-flw${flw}-cong-state.png'
	set xlabel "Time [s]"
	set ylabel "State"
	set xrange [0:$time]
	set yrange [0:4.5]
  ...
  # InFlight
	gnuplot <<- EOS
	set terminal pngcairo enhanced font "TimesNewRoman" fontscale 2.5 size 1280,960
	set output 'data/06_xx-sc${sc}-$item-${bw}-${dl}-flw${flw}-inflight.png'
	set xlabel "Time [s]"
	set ylabel "Inflight [byte]"
	set xrange [0:$time]
	plot "data/06_xx-sc${sc}-$item-${bw}-${dl}-flw${flw}-inflight.data" using 1:2 notitle with lines lc rgb "grey" lw 2 dt (10,0)
	EOS
  done
  
  cp data/06_xx-sc${sc}-$item-${bw}-${dl}-*cwnd.data /media/sf_neko9_tcpbook/ns3/data/sc${sc}/.
  cp data/06_xx-sc${sc}-$item-${bw}-${dl}-*rtt.data /media/sf_neko9_tcpbook/ns3/data/sc${sc}/.
  cp data/06_xx-sc${sc}-$item-${bw}-${dl}-*inflight.data /media/sf_neko9_tcpbook/ns3/data/sc${sc}/.
  cp data/06_xx-sc${sc}-$item-${bw}-${dl}-*cong-state.data /media/sf_neko9_tcpbook/ns3/data/sc${sc}/.
  cp data/06_xx-sc${sc}-$item-${bw}-${dl}-*throughput.data /media/sf_neko9_tcpbook/ns3/data/sc${sc}/.
  cp data/06_xx-sc${sc}-$item-${bw}-${dl}-queue-*.data /media/sf_neko9_tcpbook/ns3/data/sc${sc}/.
  cp data/06_xx-sc${sc}-$item-${bw}-${dl}-*.png /media/sf_neko9_tcpbook/ns3/data/sc${sc}/.
done
done
done
